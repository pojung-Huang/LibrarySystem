...
public interface SearchBookRepository extends JpaRepository<BookEntity, Integer> {
    ...
    @Query("SELECT new tw.ispan.librarysystem.dto.comment.SearchBookDTO(b.bookId, b.title, b.author, b.category.cName) " +
            "FROM BookEntity b WHERE " +
            "(:categoryName IS NULL OR LOWER(b.category.cName) = LOWER(:categoryName)) AND " +
            "(LOWER(b.title) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(b.author) LIKE LOWER(CONCAT('%', :keyword, '%')))")
    Page<SearchBookDTO> searchBooksByCategoryAndKeyword(@Param("categoryName") String categoryName,
